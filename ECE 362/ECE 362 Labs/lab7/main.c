...
void init_tim1(void) {
    // RCC to GPIOA
    RCC->AHBENR |= RCC_AHBENR_GPIOAEN;
    GPIOA->MODER &= ~0xff0000; // Clear vals for PA8-11
    GPIOA->MODER |= 0xaa0000; // Set PA8-11 to alt func mode
    for (int i = 8; i < 12; i++) {
        GPIOA->AFR[1] &= ~(0xf << (4*(i-8)));
        GPIOA->AFR[1] |= 0x2 << (4*(i-8));
    }
    // RCC to tim1
    ...
    TIM1->PSC = 1-1;
    TIM1->ARR = 2400-1;
//     OCM1,2,3,4 PWM MODE 1
    TIM1->CCMR1 &= ~0x7070;
    TIM1->CCMR1 |= 0x6060;
    TIM1->CCMR2 &= ~0x7070;
    TIM1->CCMR2 |= 0x6060;
    TIM1->CCMR2 |= 1<<11;   // OC4PE Channel 4
...
//===========================================================================
void setrgb(int rgb) {
    int arr_const = 2400 / 100;
    int r = (rgb & 0xff0000)>>(4*4);
    int g = (rgb & 0x00ff00)>>(4*2);
    int b = (rgb & 0x0000ff)>>(4*0);
    TIM1->CCR1 = (100 - r) * arr_const;
    TIM1->CCR2 = (100 - g) * arr_const;
    TIM1->CCR3 = (100 - b) * arr_const;
}
